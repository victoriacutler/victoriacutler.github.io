...
When we have datasheets comprehensively describing our data, we are able to more accurately identify biases for AI computer model building. Furthermore, our AI models themselves should come along with datasheets for increased transparency, better model comprehension, and ultimately, better model quality checks. [@mcnally]

AI will be as unjust as the unjustness that goes in (whether that's due to a lack of diversity, unconscious bias, or raw data that is rooted in injustice). So - back to the original question: should AI be used for decision making? Algorithms may never be perfect, but that's why it's important for our models to not be a black box.
